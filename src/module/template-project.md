...
1. `type` - 模块分类，跟 [子模块内容分类](../data/dict/2007_project_module_content_type.md) 保持一致，值为
   ...
   2. `business-docs`       需求文档
   3. `design-docs`         设计文档
   4. `backend-business`    后端业务代码
   5. `backend-system`      后端系统管理代码
   6. `frontend-business`   前端业务代码
   7. `browser-business`    浏览器端业务代码
   8. `mobile-business`     移动端业务代码
   9. `pc-business`         桌面端业务代码
   10. `help-docs`          帮助文档
...
一个文件的路径或者文件夹的路径，相对于当前项目(不是子模块)根目录的路径，但**不包含根目录**。
支持的路径信息与 [project_path_setting](../db/project_path_setting.md) 表中“支持的属性”保持一致
...
注意，此处的 `project_name` 与项目在源码托管平台上的分组名称不同，如项目名为 `wms`, 而分组名可能为 `{prefix}-wms-projects`
1. 项目名称，在创建子包或者目录时使用
2. 分组名或用户名，作为项目根目录
